...
public class ExampleFederate {
	...
	public void runFederate(String federateName) throws Exception {
		...
		try {
			URL[] modules = new URL[] { (new File("model/tmp/RestaurantProcesses.xml")).toURI().toURL(),
					(new File("model/tmp/RestaurantFood.xml")).toURI().toURL(),
					(new File("model/tmp/RestaurantDrinks.xml")).toURI().toURL() };
		...
		////////////////////////////
		URL[] joinModules = new URL[] { (new File("model/tmp/RestaurantSoup.xml")).toURI().toURL() };
	 ...
	 * the FOM (Cola - 101, Orange - 102, RootBeer - 103, Cream - 104) and it will
	 * set the number of cups to the same value as the current time.
	 * <p/>
